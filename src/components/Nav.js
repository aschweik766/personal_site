import React from 'react'
import { useState } from 'react'
import { Navbar, Container, Nav } from 'react-bootstrap'
import { Link } from 'react-scroll'
import github from '../assets/icons/icons8-github-64-white.png';
import linkedin from '../assets/icons/icons8-linkedin-circled.274ed23e113a52196919f4fb634c2959.svg'

function NavBar() {
    const [activeLink, setActiveLink] = useState('home')

    const onUpdateActiveLink = (value) => {
        setActiveLink(value)
    }

    return (
        <Navbar expand="lg" className='header'>
            <Container className='navigation'>
                <Navbar.Brand href="#home" className='name'>
                </Navbar.Brand>
                <Navbar.Collapse id="basic-navbar-nav">
                    <Nav className="me-auto">
                        <Link to='about' smooth={true} duration={1500} className={activeLink === 'about' ? 'active navbar-link' : 'navbar-link'} onClick={() => onUpdateActiveLink('about')}><h1>About</h1></Link>

                        <Link to='skills' smooth={true} duration={1500} className={activeLink === 'skills' ? 'active navbar-link' : 'navbar-link'} onClick={() => onUpdateActiveLink('skills')}><h1>Skills</h1></Link>

                        <Link to='projects' smooth={true} duration={1500} className={activeLink === 'projects' ? 'active navbar-link' : 'navbar-link'} onClick={() => onUpdateActiveLink('projects')}><h1>Projects</h1></Link>

                    </Nav>
                    <span className='navbar-text'>
                        <div className='social-icon'>
                            <a href='https://github.com/aschweik766' target='_blank' rel='noreferrer' className='github-link'><img src={github} alt='' className='social-logo'></img></a>
                            <a href='https://linkedin.com/in/ashley-schweikhard' target='_blank' rel='noreferrer' className='github-link'><img src={linkedin} alt='' className='social-logo'></img></a>
                        </div>
                        <div className='name'>Ashley Schweikhard</div>
                    </span>
                </Navbar.Collapse>
            </Container>
        </Navbar>




    )
}

export default NavBar